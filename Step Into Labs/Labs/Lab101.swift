...
struct Lab101: View {
    ...
    var body: some View {
        RealityView { content in
            ...
            let newSubject = await makeExtrudedEntity(title: "Subject", path: simplePath(), example: example)
            subject = newSubject
            spinSubject(entity: newSubject)
            content.add(newSubject)
            
        } update: { content in
            // Keep the content in sync with our selected example.
            if let subject {
                // Ensure only one subject is present.
                if !content.entities.contains(where: { $0.id == subject.id }) {
                    content.add(subject)
                }
            }
        }
        .onChange(of: example) { _, newValue in
            Task { @MainActor in
                let newSubject = await makeExtrudedEntity(title: "Subject", path: simplePath(), example: newValue)
                if let old = subject {
    ...
    private func makeExtrudedEntity(title: String, path: Path, example: ExtrusionExample) async -> ModelEntity {
